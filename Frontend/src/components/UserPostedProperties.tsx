...
const UserPostedProperties: React.FC = () => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      let res = await fetch(`https://landsdevelop25.onrender.com/api/user-properties-by-phone/${phone}`);
      
      if (!res.ok) {
        // Fallback to the old method if the new endpoint doesn't exist yet
        res = await fetch('https://landsdevelop25.onrender.com/api/all');
  ...
  const handleCloseDeal = async (id: string) => {
    ...
    try {
      const res = await fetch(`https://landsdevelop25.onrender.com/api/properties/${id}/close`, {
        method: 'PATCH',
        headers: {
              ...
              {userProperties.map((project) => (
                ...
                >
                  <div className="relative h-48">
                    <img
                      src={project.imageUrl ? `https://landsdevelop25.onrender.com${project.imageUrl}` : 'https://via.placeholder.com/400x200?text=No+Image'}
